...
def locale_lookup(path, default_locale, key):
    ...
    with open(locale_file) as fh:
        ...
        if key in ldata:
            return ldata[key]["message"]
    logging.error("ERROR: missing key: %s", key)
    return ""
...
def show_permissions_info(permlist):
    if permlist and isinstance(permlist, list):
    ...
    for profile in profiles:
        ...
        for ext in os.listdir(path):
            ...
            for ver in os.listdir(ext_path):
                ...
                permissions = list_of_permissions(data)
                logging.info("     Name: %s\n     Version: %s\n     Description: %s\n     Permissions: %s" %
                             (name, version, description, ", ".join(permissions)))
                if args.permissions_info:
                    show_permissions_info(permissions)
                if can is not None:
                    if name not in can:
                        logging.warning("WARNING: this extension is not allowed or might be unwanted!")
                        not_in_can.append(name)
                        res = False
                if must is not None and name in must:
                        must.remove(name)
                logging.info("\n")
        if not_in_can:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--permissions_info", "-p", "--perm", help="Show information to every active permission",
                        action="store_true")
